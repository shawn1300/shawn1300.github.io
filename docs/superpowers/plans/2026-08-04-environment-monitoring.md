...
- [ ] Add `browser_bootstrap.py` as a local fallback that accepts at most 64 KiB of hidden, one-line Xiaomi `xiaomiio` service-login JSON, strips an optional `&&&START&&&` XSSI prefix, requires the remaining input to be a JSON object with authenticated `code=0`, extracts only `userId`, `ssecurity`, and a one-time `location`, and never logs or persists the source response or `passToken`.
- [ ] Validate the one-time completion URL using a strict HTTPS Xiaomi host allowlist with no user info or non-default port, consume it once in a fresh local session to obtain `serviceToken`, then reuse the normal device selection, real-read probe, and credential writer.
...
- [ ] Test overlong/malformed input, unauthenticated status, missing fields, hostile completion URLs, used locations without `serviceToken`, response redaction, and the shared successful probe path. Remind the user to clear the clipboard without reading or overwriting it automatically.
