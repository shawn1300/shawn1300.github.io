...
- [ ] Add `browser_bootstrap.py` as a local fallback that accepts at most 64 KiB of hidden, one-line Xiaomi `xiaomiio` service-login JSON, strips `&&&START&&&`, requires authenticated `code=0`, extracts only `userId`, `ssecurity`, and a one-time `location`, and never logs or persists the source response or `passToken`.
